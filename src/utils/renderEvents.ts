...
import { DataAttributeHTMLElement } from './constants';
...
export function renderDayEvents(dayEvents:Event[], eventsContainer:HTMLElement, dayContainer:HTMLElement){ 
    ...
    eventsToRender.forEach((ev:Event)=>{   
    ...
    if(ev.endDate) eventDetailsInnerTemplate += `<p>endDate: ${ev.endDate}</p>`
    if(ev.reminder) eventDetailsInnerTemplate += `<p>Reminder: ${ev.reminder}</p>`
  ...
  if(dayEvents.length > 3){
    ...
    setTimeout(() => {
        ...
        const popoverTriggerEl2 = document.querySelector('[data-type="view-day-events-btn"]:not([data-trigger="popover"])') as DataAttributeHTMLElement;
