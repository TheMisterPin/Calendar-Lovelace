...
export function renderDayEvents(dayEvents:Event[], eventsContainer:HTMLElement, dayContainer:HTMLElement, milliseconds:number){ 
	...
	eventsToRender.forEach((event:Event)=>{   
		...
		if(event.milliseconds < milliseconds) eventNameEl.classList.add('expired-event')
